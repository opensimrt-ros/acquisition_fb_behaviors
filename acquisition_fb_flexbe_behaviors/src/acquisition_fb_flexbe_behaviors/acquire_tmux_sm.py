...
@author: frekle
'''
class acquire_tmuxSM(Behavior):
	'''
	acquire with a more complicated structure
	'''


	def __init__(self):
		super(acquire_tmuxSM, self).__init__()
		self.name = 'acquire_tmux'
		...
		self.add_parameter('run_nodes', False)
	...
	def create(self):
		...
		with _sm_recording_trial_1:
			...
			OperatableStateMachine.add('start_recording_srv',
										MultiServiceCallState(multi_service_list="/ik_lowerbody_node", predicate="/start_recording", prefix=""),
			...
			OperatableStateMachine.add('stop_recording_srv',
										MultiServiceCallState(multi_service_list="/ik_lowerbody_node", predicate="/stop_recording", prefix=""),
										transitions={'done': 'write_sto_srv', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})

			# x:30 y:343
			OperatableStateMachine.add('write_sto_srv',
										MultiServiceCallState(multi_service_list="/ik_lowerbody_node", predicate="/write_sto", prefix=""),
